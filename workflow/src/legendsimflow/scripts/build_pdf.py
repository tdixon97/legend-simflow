...
def build_pdf() -> None:
    ...
    log = build_log(config, args.log_file)
    ...
    rconfig = Props.read_from(args.config)
    ...
    log.info("... computing number of simulated primaries from raw files")
    if args.raw_files:
            ...
            for array in pytree.iterate(step_size="100 MB"):
                ...
                for _cut_name, _cut_dict in rconfig["cuts"].items():
                    ...
                    if _cut_dict["is_sum"] is False and _cut_dict["is_2d"] is False:
                        ...
                        "lar_energy" not in _cut_dict
                        or _cut_dict["lar_energy"] is False
                    ):
